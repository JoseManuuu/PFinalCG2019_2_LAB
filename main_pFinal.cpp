...
void ciudad ()
{
		...
		glPushMatrix(); //Casa01
			...
			glScalef(6,5.0,6);
			glDisable(GL_LIGHTING);
			fig5.prisma2(1.0, 1.0, 1.0, text6.GLindex, 0);
			glEnable(GL_LIGHTING);
		glPopMatrix();
...
void casa() {
	glPushMatrix();
		...
		fig1.prisma3(10, 18, 15, textFachada.GLindex, textTabique.GLindex, textPiso.GLindex, textTecho.GLindex);
	glPopMatrix();
...
void television() {
	...
	glPushMatrix();
		...
		if (pantalla) {			
			if(i){
				fig1.prisma2(3, 4, 0.1, textNegro.GLindex, textPantalla[25].GLindex);
				i = !i;
			}else{
				fig1.prisma2(3, 4, 0.1, textNegro.GLindex, textPantalla[26].GLindex);
				i = !i;
			}
							
		}
		else {
			fig1.prisma2(3, 4, 0.1, textNegro.GLindex, textPantalla[0].GLindex);
		}
		glPopMatrix();
}
...
int main ( int argc, char** argv )   // Main Function
{
  ...
  glutCreateWindow    ("Proyecto Final LAB CG 2019_2"); // Nombre de la Ventana
